...
SHI Discover is an enterprise product from SHI International Corp. that requires permissions and software to run.
...
!!! info "Permissions Usage"
    SHI Discover will never change your service configuration in any way during an audit. All permissions are used only for data retrieval and are the set of least privilege possible for the data that is being gathered.
