...
Moot Security Management is an enterprise product from Moot, Inc. that requires specific licenses and configurations to deploy and use.
...
- [X] Deploying User has `Global Admin Rights` (for [manual deployment](/Getting-Started/Deployment/Manual-Deployment/) only)
...
- [X] Certificate Authentication is [disabled in the Entra ID authentication](https://learn.microsoft.com/en-us/azure/active-directory/authentication/how-to-certificate-based-authentication#step-2-enable-cba-on-the-tenant) methods for MSM's ESM, SSM and PSM root security groups
...
- [X] Devices to be managed through MSM need to be either Hybrid or Cloud only joined
...
- [X] `M5` [or equivalent](https://go.microsoft.com/fwlink/?linkid=2139145){:target="_blank"} licenses are purchased and enabled in the target tenant
- [X] Secure Core Certified hardware. Please see the [hardware selection](/Reference/Architecture/Hardware-Selection) documentation for details
